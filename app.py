...
def generar_grafico_km(df, curvas, incluir_PO, unidad_tiempo, intervalo_x, x_max, fecha_fin_estudio='2018-12-31'):
    df_km = df.copy()
    ...
    grupos_validos = ['PP', 'PX'] + (['PO'] if incluir_PO else [])
    df_km = df_km[df_km['GRUPO'].isin(grupos_validos)].copy()
    
    if df_km.empty:
        return None

    df_km['EVENTO'] = np.where(df_km['GRUPO'] == 'PP', 1, 0)
    ...
    fig.update_layout(
        title=None, # Título eliminado de la gráfica para maximizar espacio
...
# Variables de estado para recordar el gráfico anterior
...
# Diccionario de presets clínicos actualizado
presets = {
...
with st.sidebar:
    ...
    if seleccion_preset == "Personalizado":
        todas_las_curvas = ['global', 'operados', 'no_operados', 'quimioterapia', 'sin_quimio', 'ambos', 'solo_cirugia', 'solo_quimio', 'ninguno', 'fonasa', 'isapre', 'ffaa', 'tramo_a', 'tramo_b', 'tramo_c', 'tramo_d', 'diag_publico', 'diag_privado', 'cirugia_publico', 'cirugia_privado']
